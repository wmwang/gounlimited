...
do
	...
	d=`date +%s`
	echo d is ${d}
	curl ${c} | grep -e 'm3u8'| sed "s/m3u8.*/m3u8/g"| sed "s/.*http/http/g"|  > ${d}
	...
	if ! [ -f ./`echo ${c} |sed 's/.*\///g'` ]; then
		...
		ffmpeg -protocol_whitelist file,https,https,tcp,tls,crypto -i ${d} -c copy `echo $line |sed 's/.*\///g'`_`echo ${c} |sed 's/.*\///g'`
	fi
	rm ${d}
